...
def Thomas(lower_diag, main_diag, upper_diag, solutions):
    ...
    for i in range(1, n - 1):
        c_trenutno[i] = c[i]/(b[i] - a[i]*c_trenutno[i])
        d_trenutno[i] = (d[i] - a[i]*d_trenutno[i - 1]/(b[i] -a[i]*c_trenutno[i - 1]))
    d_trenutno[n - 1] = (d[n - 1] - a[n - 1]*d_trenutno[n - 2])/(b[n - 1] - a[n - 1]*c_trenutno[n - 2])
